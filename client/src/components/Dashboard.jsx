...
export default function Dashboard() {
  ...
  try {
    ...
    const response = await axios.get(`https://lab-gen.ru/api/reports?clientId=${clientId}`);
    ...
    const loadData = async () => {
      try {
        ...
        const [reportsRes, templatesRes] = await Promise.all([
          axios.get('https://lab-gen.ru/api/reports', { params: { clientId } }),
          axios.get('https://lab-gen.ru/api/templates')
        ]);
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(`https://lab-gen.ru/api/reports/${modalData.id}`);
  ...
  const handleDuplicate = async (id) => {
    setActiveMenuId(null);
    try {
      await axios.post(`https://lab-gen.ru/api/reports/duplicate/${id}`);
  ...
  const handleDownload = async (id) => {
    setActiveMenuId(null);
    try {
      const reportRes = await axios.get(`https://lab-gen.ru/api/reports/${id}`);
      const { meta_data, content_data } = reportRes.data;

      const response = await axios.post('https://lab-gen.ru/api/generate-docx', {
        meta: meta_data,
